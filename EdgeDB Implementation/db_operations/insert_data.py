...
def extract_concatenated_notes(notes_str):
    # Function to determine the key for grouping (uppercase letter or not)
    key_func = lambda x: x.islower()

    # Grouping characters based on whether they are lowercase
    grouped = itertools.groupby(notes_str, key_func)

    # Constructing notes from groups
    notes = []
    for is_lower, group in grouped:
        if is_lower:
            # Append grouped lowercase characters to the last note
            notes[-1] += ''.join(group)
        else:
            # Start a new note for uppercase characters
